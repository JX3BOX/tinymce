...
function fillUserFields() {
    ...
    // 填充上传模式的字段
    document.getElementById("audio-author-upload").value = author;
    document.getElementById("audio-userid-upload").value = userId;

    // 填充链接模式的字段
    document.getElementById("audio-author-link").value = author;
    document.getElementById("audio-userid-link").value = userId;
}

// 初始化输入验证
function initInputValidation() {
    // 魔盒ID字段只允许数字
    const userIdInputs = ["audio-userid-upload", "audio-userid-link"];
...
function init() {
    ...
    document.getElementById("insert").onclick = () => {
        ...
        document.getElementById(`author-error${errorSuffix}`).classList.add("hidden");
        document.getElementById(`audio-author${errorSuffix}`).classList.remove("error");
        document.getElementById(`userid-error${errorSuffix}`).classList.add("hidden");
        document.getElementById(`audio-userid${errorSuffix}`).classList.remove("error");
        ...
        // 根据模式获取对应的name、author和user_id字段
        const name = mode === "upload" ? document.getElementById("audio-name-upload").value.trim() : document.getElementById("audio-name-link").value.trim();
        const author = mode === "upload" ? document.getElementById("audio-author-upload").value.trim() : document.getElementById("audio-author-link").value.trim();
        const userId = mode === "upload" ? document.getElementById("audio-userid-upload").value.trim() : document.getElementById("audio-userid-link").value.trim();
        ...
        if (!author) {
            document.getElementById(`author-error${errorSuffix}`).classList.remove("hidden");
            document.getElementById(`audio-author${errorSuffix}`).classList.add("error");
            hasError = true;
        }
        
        if (!userId) {
            document.getElementById(`userid-error${errorSuffix}`).classList.remove("hidden");
            document.getElementById(`audio-userid${errorSuffix}`).classList.add("error");
            hasError = true;
        }
