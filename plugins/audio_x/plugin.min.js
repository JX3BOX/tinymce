tinymce.PluginManager.add("audio_x", function (editor, url) {
    // 存储token供iframe使用
    ...
    const openDialog = () => {
        ...
        const dialogUrl = `${root}/plugins/audio_x/audio-dialog/index.html?root=${encodeURIComponent(root)}`;
    ...
    // 注册一个工具栏按钮名称
    editor.ui.registry.addButton("audio_x", {
        icon: "audio_x",
        tooltip: "插入音频",
        // text: '插入音频',
        onAction: function () {
    ...
    window.addEventListener("message", (e) => {
        ...
        if (e.data?.source === "tinymce-voice" && e.data.type === "insert") {
            let { name, author, user_id, src, avatar } = e.data.payload;
            // 插入音频HTML，使用 w-audio 和 e-audio 类名
            const content = `name|${name || ''};author|${author || ''};user_id|${user_id || ''};src|${src || ''};avatar|${avatar || ''}`;
